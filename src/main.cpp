...
#if defined(NDEBUG)
# error "IvugeoEvolutionCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc7a4b136e742e05892e84b36be18a98da60972ad76993f071a1184369ca7bd0f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IvugeoEvolutionCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IvugeoEvolutionCoin Signed Message:\n";
...
{
    // IvugeoEvolutionCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan2 = 1 * 60; // IvugeoEvolutionCoin: 1 minutes
static const int64 nTargetSpacing = 1 * 60; // IvugeoEvolutionCoin: 1 minutes
...
static const int64 nBlockChangeHeight = 16000;
static const int64 nAdjustDiffMaxUpOriginal = 4;
static const int64 nTargetTimespanOriginal = 10 * 30; // IvugeoEvolutionCoin: 5 minutes
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
	...
	if (pindexBest->nHeight+1 < nBlockChangeHeight)
	{
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (pindexLast->nHeight+1 < nBlockChangeHeight)
	{
    nTargetTimespan = nTargetTimespanOriginal; 
    ...
    // IvugeoEvolutionCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // seconds
...
{		
      ...
      if (pindexLast->nHeight+1 < nBlockChangeHeight) { return GetNextWorkRequired_V1(pindexLast, pblock); }
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if ((nHeight > nBlockChangeHeight) && (GetBlockTime() <= pindexPrev->GetBlockTime() - 15 * 60)) {
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc7a4b136e742e05892e84b36be18a98da60972ad76993f071a1184369ca7bd0f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Worldwide 01/Jan/2015 Start of the year 2015";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1420074000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1128005;
        ...
        assert(block.hashMerkleRoot == uint256("0x83634214128b60bc36984c713b5908791e6398037524dd94246b3c616c4fa3dd"));
...
unsigned char pchMessageStart[4] = { 0xeb, 0xa0, 0xd6, 0xcb }; // IvugeoEvolutionCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("ivugeoevolutioncoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ivugeoevolutioncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ivugeoevolutioncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ivugeoevolutioncoinMiner(CWallet *pwallet)
{
    printf("ivugeoevolutioncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ivugeoevolutioncoin-miner");
    ...
    try { loop {
        ...
        printf("Running ivugeoevolutioncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ivugeoevolutioncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ivugeoevolutioncoinMiner, pwallet));
}
